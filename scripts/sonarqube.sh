...
sudo systemctl start mysqld && systemctl enable  mysqld  && sudo cd /gcpterraform/scripts
sudo chmod 777  /gcpterraform/scripts/* 
sudo sh /gcpterraform/scripts/mysqlroot_passwdreset 
sudo sh /gcpterraform/scripts/mysqlcreate_db_user --host=localhost --database=sonardb --user=sonaruser < /gcpterraform/scripts/mysqlroot_passwd
